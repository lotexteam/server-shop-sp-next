...
- ✅ **Caddy на стенде перечитан** (2026-09-23, вечер): Googlebot получает 200 на `/`,
  `/catalog`, `/contacts` (было 404 от Laravel), `@static_media` применился
  (`max-age=86400, stale-while-revalidate=604800`). Корень был не в «забыли reload»,
  а в невалидном Caddyfile (`handle` принимает один матчер) — разбор:
  `server-shop/docs/ai-agent/updates/2026-09-23-caddy-reload-after-frontend.md`.
- ✅ **P1.1 — кэш серверных данных**: `src/lib/server-data.ts` вместо `cache:"no-store"`
  ...
  До этого каждый SSR-запрос заново обходил API (TTFB упирался в бэкенд).
  Замер главной (mobile UA, transfer): HTML 15.3 КБ gzip (decoded 84 КБ, из них
  56.9 КБ инлайн-скриптов), JS 263 КБ в 18 чанках, CSS 12.9 КБ, LCP-медиа 18.8 КБ preloaded.
- ✅ **PageSpeed mobile (Lighthouse 13.4.1, Moto G Power, Slow 4G) — разбор и фиксы**:
  ...
  т.е. `isAuditBot()` не срабатывал, и 1.4 МБ конкурировали за канал с LCP.
- ⏳ Осталось по mobile: CLS (нужен culprit-трейс), полифилы Baseline 13.4 КиБ
  (лечится `browserslist` в package.json — требует решения по поддержке старых
  браузеров), видео 1.4 МБ на мобильном (решить: постер вместо видео или сжатие),
  a11y 91 (контраст, области касания, порядок заголовков, alt).
- ✅ **Личный кабинет → Настройки:** строка «Двухфакторная аутентификация» убрана —
  у покупателя в бэкенде нет ни endpoints, ни полей под неё (2FA TOTP есть только
  у staff в `/api/v1/admin`, см. `server-shop/docs/ADMIN-API.md`), переключатель был
  декоративным. «Новости и акции» привязана к реальной подписке:
  `GET /newsletter/status` (auth) при открытии кабинета, `POST /newsletter/subscribe`
  (`source: "account"`) / `POST /newsletter/unsubscribe` (auth) по переключателю —
  новые `fetchNewsletterStatus` / `subscribeNewsletter` / `unsubscribeNewsletter`
  в `src/lib/api.ts`, поведение 1:1 как в sale-next. Остальные два переключателя
  («Email-уведомления о заказах», «Уведомления о начислении бонусов») по-прежнему
  декоративны: полей под них в профиле нет.
- ⏳ Осталось: HTML/ISR-кэш самих страниц (P1.1 — нужен ответ про персонализацию
  корзины/сравнения), TTFB-замер на стенде, P1.5 шрифты; настоящий 410 (сейчас 404+noindex).
